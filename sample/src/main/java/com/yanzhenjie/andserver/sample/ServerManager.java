...
public class ServerManager extends BroadcastReceiver {
    ...
    private static String getAction(Context context) {
        return context.getPackageName() + ".andserver.receiver";
    }

    private static final String CMD_KEY = "CMD_KEY";
    private static final String MESSAGE_KEY = "MESSAGE_KEY";
    private static final String LOCAL_IP_KEY = "LOCAL_IP_KEY";
    private static final String PUBLIC_IP_KEY = "PUBLIC_IP_KEY";
    private static final String IS_IPV6_KEY = "IS_IPV6_KEY";
     ...
     */
    public static void onServerStart(Context context, String localIp, String publicIp, boolean isPublicIPv6, int port) {
        Intent broadcast = new Intent(getAction(context));
        broadcast.putExtra(CMD_KEY, CMD_VALUE_START);
        broadcast.putExtra(LOCAL_IP_KEY, localIp);
        broadcast.putExtra(PUBLIC_IP_KEY, publicIp);
        broadcast.putExtra(IS_IPV6_KEY, isPublicIPv6);
        broadcast.putExtra(PORT_KEY, port);
    ...
    public void register() {
        IntentFilter filter = new IntentFilter(getAction(mActivity));
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (getAction(context).equals(action)) {
            int cmd = intent.getIntExtra(CMD_KEY, 0);
            switch (cmd) {
                case CMD_VALUE_START: {
                    String localIp = intent.getStringExtra(LOCAL_IP_KEY);
                    String publicIp = intent.getStringExtra(PUBLIC_IP_KEY);
                    boolean isPublicIPv6 = intent.getBooleanExtra(IS_IPV6_KEY, false);
                    int port = intent.getIntExtra(PORT_KEY, -1);
                    mActivity.onServerStart(localIp, publicIp, isPublicIPv6, port);
                    break;
                }
